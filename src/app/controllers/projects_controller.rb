...
#    dim_txt = (!params[:dim3] || session[:viz_params]['dim3'].to_s == '') ? '2d' : '3d'
    @h_data = {}
    ['2d', '3d'].each do |dim|
      output_json = tmp_dir + "output.#{dim}.json"
      if !File.exist? output_json
        output_json = tmp_dir + "output.json"
      end
      @data_json = File.read(output_json)
      #    @data_json = File.read(tmp_dir + "output.json") 
      @h_ori_data = JSON.parse(@data_json)
      @h_data[dim]={'text' => @h_ori_data['text']}
      session[:viz_params]['dim3']='1' if !@h_ori_data["PC" + (params[:dim3] || session[:viz_params]['dim3'].to_s)]
      @h_data[dim]['x']=@h_ori_data["PC" + (params[:dim1] || session[:viz_params]['dim1'].to_s)]
      @h_data[dim]['y']=@h_ori_data["PC" + (params[:dim2] || session[:viz_params]['dim2'].to_s)]
      @h_data[dim]['z']=@h_ori_data["PC" + (params[:dim3] || session[:viz_params]['dim3'].to_s)] #if params[:plot_type]=='3d'
    end
  end
  ...
  def create_traces(h_results, trace_names, dim)
  
    traces = []
    @h_data[dim]['text'].each_index do |i|
     
      text =  @h_data[dim]['text'][i]
      if h_results[text]
        x = @h_data[dim]['x'][i]
        y = @h_data[dim]['y'][i]
        z = @h_data[dim]['z'][i]
  ...
  #      if params[:attrs].to_json != @pdr.attrs_json and !params[:active_item]
        ...
        if (!params[:attrs][:dim1] and (!@pdr.status_id or [4,5].include?(@pdr.status_id)) and @pdr.dim_reduction_id < 5) or (params[:attrs].to_json != @pdr.attrs_json and !params[:active_item]) #or params[:dataset] != h_tmp_attrs['dataset']
          ...
          if analyzable?(@project) and (@pdr.dim_reduction_id < 5 or owner?(@project))
            ...
            ['2d', '3d'].each do |dim|
              #            dim_txt = (!session[:viz_params]['dim3'] || session[:viz_params]['dim3'] == '') ? '2d' : '3d'
              output_json = tmp_dir + "output.#{dim}.json"
              output_json = tmp_dir + "output.json" if !File.exist? output_json
              #            output_json = tmp_dir + "output.json"
      ...
      elsif @pdr.dim_reduction_id == 8
        volcano_proteomics_plot(@pdr, project_dir)
      end
    end
...
#    @button = "<button id='reset_visualization_" + params[:dr_id] + "' class='replot btn btn-primary' style=''>Plot</button>"
  ...
  def volcano_proteomics_plot(pdr, project_dir)
  

    @comparisons = []
    input_file =  Course.where(:project_key => @project.key).first
    #matrix_file =  Pathname.new(APP_CONFIG[:upload_data_dir]) + 'courses' + input_file.id.to_s + input_file.upload_file_name

    metadata_file = Pathname.new(APP_CONFIG[:upload_data_dir]) + 'courses' + input_file.id.to_s + (input_file.upload_file_name + ".metadata") #@project.upload_path + ".metadata"
    unselected_trace = {:x => [], :y =>  [], :text => [], mode: 'markers', type: 'scatter', name: 'Unselected'}
    selected_trace = {:x => [], :y => [], :text => [], mode: 'markers', type: 'scatter', name: 'Selected'}

    session[:current_volcano] = params[:comparison] if params[:comparison]

    [:volcano_pval_threshold, :volcano_logfc_threshold].each do |e|
      session[e] = params[e].to_f if params[e]
    end
    ...
    File.open(metadata_file, 'r') do |f|
      ...
      @comparisons = header.map{|e| (m = e.match(/^(.+?)_Results_logFC$/)) ? m[1] : nil}.compact
      session[:current_volcano] ||= @comparisons.first

      #get position for current_volcano
      logfc_idx = header.find_index("#{session[:current_volcano]}_Results_logFC")
      pval_idx = logfc_idx+1

      while (l = f.gets) do 
        t = l.chomp.split("\t")
        @protein_list.push t[0]
        if t[logfc_idx].to_f.abs >= session[:volcano_logfc_threshold] and t[pval_idx].to_f <= session[:volcano_pval_threshold] 
          selected_trace[:x].push t[logfc_idx].to_f
          selected_trace[:y].push (t[pval_idx].to_f > 0) ? -Math.log10(t[pval_idx].to_f) : 1000
          selected_trace[:text].push t[0]
        else
          unselected_trace[:x].push t[logfc_idx].to_f
          unselected_trace[:y].push (t[pval_idx].to_f > 0) ? -Math.log10(t[pval_idx].to_f) : 1000
          unselected_trace[:text].push t[0]
        end
      end
    end

    @traces = [selected_trace, unselected_trace]
    @layout = {
  ...
  def standard_plot(pdr, project_dir)
    ...
    ['2d', '3d'].each do |dim|
      ...
      if params[:color_by] == 'gene_text' and params[:gene_text] and params[:gene_text] != ''
        ...
        traces = create_traces(h_batches, trace_names, dim)
        ...
        traces = create_traces(h_results['clusters'], trace_names, dim)
...
#        end
  ...
  def get_file
    ...
    if params[:step] and c= params[:step].classify and ['GeneSet'].include?(c)
      obj = c.constantize 
    end
  ...
  #  settings[:public_limit]=10
    public_base_search = (!settings[:public_free_text].empty?) ? Project.where("lower(name) ~ ?", settings[:public_free_text].downcase) : Project
...
#    h[:user_id] = 1 if !current_user 
    ...
    base_search = (!settings[:free_text].empty?) ? Project.joins("join users on (users.id = user_id)").where("lower(name) ~ ? or key ~ ? or users.email ~ ?", settings[:free_text].downcase, settings[:free_text].downcase, settings[:free_text].downcase) : Project
  ...
  def index
    ...
    Organism.all.map{|o| @h_organisms[o.id]=o}
     ...
     respond_to do |format|
      ...
      format.json { render json: {:public => @public_projects, :private => @projects} }
    end
  ...
  def show
    ...
    if @project
      ...
      if readable? @project
        ...
        ### setup selections
        session[:volcano_logfc_threshold] ||= 1.5
        session[:volcano_pval_threshold] ||= 0.05
        session[:selections]={}
        if readable?(@project)
          project_dir =  Pathname.new(APP_CONFIG[:user_data_dir]) + @project.user_id.to_s + @project.key
  ...
  def create
    ...
    respond_to do |format|
      if input_file and @project.save
        ...
        header = `head #{upload_path}`
        if header.match(/^Accession\tProtein.Group.IDs\tProtein.Group.Accessions\tGene.names\tDescription\tNumber.of.Peptides\tNumber.of.Unique.Peptides\tNumber.of.PSMs\tBiological.Process\tCellular.Component\tMolecular.Function\tGene.Symbol\tWikiPathways\tKEGG.Pathways\tReactome.Pathways/)
          `ruby lib/ms2asap.rb #{upload_path}`
          @project.update_attribute(:project_type_id, 2)
        end
...
#    if editable? p
 ...
 #   end
  ...
  def project_params
    ...
    params.fetch(:project).permit(:name, :key, :organism_id, :group_filename, :input_filename, :status_id, :duration, :step_id, :filter_method_id, :norm_id, :parsing_attrs_json, :filter_method_attrs_json, :norm_attrs_json, :public, :pmid, :diff_expr_filter_json, :gene_enrichment_filter_json, :read_access, :write_access, :project_type_id)
  end
end
