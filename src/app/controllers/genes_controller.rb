...
#    h_matches = {}
    gene_list.each do |g|
      h_unmapped_genes[g[0]] = 1 if g[1] == ''
      g.each do |identifier|
    ...
    #    gene_list
  #  gene_names = []
   # gene_names.push(GeneName.where("organism_id = ? and lower(value) = ?", params[:organism_id], params[:term].downcase).order("value asc").limit(20).all)#.map{|e| e.gene_id}.uniq
  #  gene_names.push(GeneName.where("organism_id = ? and lower(value) ~ ?", params[:organism_id], ("^" + params[:term].downcase)))#.map{|e| e.gene_id}.uniq
   ...
   # final_list = Gene.where(:id => gene_ids)
    ...
    j = 0
   h_unmapped_genes.each_key do |unmapped_identifier|
      if unmapped_identifier.match(/#{params[:term]}/i)
        to_render.push(:id => nil, :label => unmapped_identifier)
        j+=1
      end
      break  if j> 10
    end
