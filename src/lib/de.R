...
data.correct <- length(which(sign(data.out$AvCountG1 - data.out$AvCountG2) == sign(data.out$logFC)))
data.incorrect <- nrow(data.out) - data.correct
...
write(toJSON(as.list(data.out.up), method="C", auto_unbox=F, digits = NA), file = paste0(output.folder,"/output.up.json"), append=F)
write(toJSON(as.list(data.out.down), method="C", auto_unbox=F, digits = NA), file = paste0(output.folder,"/output.down.json"), append=F)
