...
### Load data
#input.file = "C:/Users/gardeux/Dropbox/ASAP/TestData/bigdata.tab"
data.parsed = fread(input.file, sep="\t", header=T, colClasses=c(Genes="character"), check.names=F, stringsAsFactors=F)
data.warnings = NULL
data.errors = NULL
data.kept.genes <- NULL
expressed.genes.per.sample <- rep(0, ncol(data.parsed) - 1)
...
if (algorithm == "none"){ # Nothing to do?
  ...
  if(nb.cells.detected > ncol(data.parsed)) error.json(paste0("'Min Detected' should be smaller than the total number of cells/samples i.e. <=",ncol(data.parsed)))
  expressed.genes.per.sample <- rep(0, ncol(data.parsed) - 1)
  cpm.filtering <- function(x)
  {
      s <- sum(x)
      if(s == 0) { return(F)
      } else { data.cpm <- ((x/s)*1000000 > nb.counts.per.cell) }
      expressed.genes.per.sample <<- expressed.genes.per.sample + data.cpm
      if(sum(data.cpm) >= nb.cells.detected) return(T)
      return(F)
  }
  data.kept.genes <- apply(data.parsed[,2:ncol(data.parsed)], 1, cpm.filtering)
  data.parsed = data.parsed[data.kept.genes,]
...
if(nrow(data.parsed) == 0) error.json("The output matrix has no more genes. You should put less stringent thresholds.")

#png(paste0(output.folder,"/boxplot.png"), width=(560 + min(10000, ncol(data.parsed) * 6.5)), height=600, type="cairo")
#boxplot(data.parsed[,2:ncol(data.parsed)], outline=F, las=2, ylab="Raw expression / count")
#dev.off()

png(paste0(output.folder,"/barplot.png"), width=(560 + min(10000, ncol(data.parsed) * 6.5)), height=600, type="cairo")
barplot(expressed.genes.per.sample, las=2, ylab=ycol)
dev.off()

stats = list()
stats$nber_genes = nrow(data.parsed)
stats$nber_cells = ncol(data.parsed)
stats$nber_zeros = length(which(data.parsed == 0))
stats$list_plots = data.plots
stats$with_negatives = !all(data.parsed >= 0)
stats$warnings = as.list(c(data.warnings, data.errors))
write(toJSON(stats, method="C", auto_unbox=T), file = paste0(output.folder,"/output.json"), append=F)

data.parsed.cols = colnames(data.parsed)
fwrite(data.parsed, file=paste0(output.folder,"/output.tab"), sep="\t", row.names = F, quote=F, append = F)
